 ...
 */
function mergeFilesLinux(dir, out, segments, initFileName = "seg_0.mp4") {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      execSync(
        `for i in seg_{1..${segments}}.mp4; do cat "$i"; done >> "${out}"`,
...
/**
 * Merges segments using windows copy
 * @param {string} dir path to directory that contains segments
 * @param {string} out path to merged file
 * @param {number} segments number of segments to merge
 */
function mergeFilesWindows(dir, out, segments) {
  return new Promise(async (resolve, reject) => {
    const platform = process.platform;
    if (platform !== "win32") {
      console.error(`${platform} is not supported!`);
    ...
    for (var i = 0; i < segments; i++) {
      segmentsArray.push(`seg_${i}.mp4`);
    }
    try {
      execSync(`copy /b ${segmentsArray.join("+")} "${out}"`, {
        cwd: dir,
        encoding: "utf8",
      });
    } catch (e) {
...
function mergeFiles(dir, out, segments, initFileName = "seg_0.mp4") {
  if (process.platform === "win32") {
    return mergeFilesWindows(dir, out, segments);
  } else if (process.platform === "linux") {
    return mergeFilesLinux(dir, out, segments, initFileName);
  } else {
...
function processSegments(type, listPath, NOS, key) {
  var DIR, ENC, DEC;
  ...
  return new Promise(async (resolve, reject) => {
    ...
    // await downloadFiles(listPath).catch(console.error);
